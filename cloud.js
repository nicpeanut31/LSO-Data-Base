(() => {
  ...
  try {
    ...
    if (Array.isArray(pending)) pending.forEach((column) => {
      if ([...ARRAY_COLUMNS, 'settings'].includes(column)) dirtyVersions.set(column, 1);
    });
  } catch {
  ...
  function dispatchDomainChange(key, source = 'cloud') {
    ...
    if (['lso_events_v2', 'lso_attendance_v2', 'lso_instruments_v2', 'lso_activity_log_v2', 'lso_system_settings_v2'].includes(key)) {
